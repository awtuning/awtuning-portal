...
export default function TicketsPage() {
  ...
  useEffect(() => {
    const loadTickets = async () => {
      ...
      const { data, error } = await supabase
        .from("requests")
        .select("id, registration_or_vin, vehicle_make, vehicle_model, status")
        .eq("user_id", user.id)
  ...
  return (
    <main>
      <h1 className="text-3xl font-bold text-red-600 mb-8">Tickets</h1>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                ...
                <div>
                  ...
                  <span
                    className={`inline-block text-sm px-3 py-1 rounded-full ${getStatusStyle(ticket.status)}`}
                  >
